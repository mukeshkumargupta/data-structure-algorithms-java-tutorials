...
public class WordSearch {
    ...
    private boolean isSafe(int i, int j, int r, int c, int index, char[][] board, String word) {
        return i >= 0 && i < r && j >= 0  && j < c && board[i][j] != '$' && board[i][j] == word.charAt(index);
    }
    ...
    private boolean dfs(char[][] board, String word, int row, int col, int index) {
        ...
        if (isSafe(row, col, board.length, board[0].length, index, board, word) == false) {
            return false;
