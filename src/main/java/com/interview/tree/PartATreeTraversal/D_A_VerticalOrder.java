 ...
 */
public class D_A_VerticalOrder {
    //////////////////////////////////////////////////////////////////////////////////
    // Using bfs approach
    class Point {
        TreeNode root;
...
Memory Usage: 39.5 MB, less than 36.20% of Java online submissions for Vertical Order Traversal of a Binary Tree.
    ...
    class TreeNodeDetails  {
        Point p;
        TreeNode treeNode;
        TreeNodeDetails (Point p, TreeNode treeNode) {
            this.p = p;
    ...
    private void dfs(TreeNode root, Point p, Map<Point, PriorityQueue<TreeNodeDetails>> verticalOrderMap) {
        if (root == null) {
            return;
        }

        if (!verticalOrderMap.containsKey(p)) {
            PriorityQueue<TreeNodeDetails> pq = new PriorityQueue<>( (a, b)-> {
                if (a.p.row != b.p.row ) {
            ...
            verticalOrderMap.put(p, pq);
        }

        PriorityQueue<TreeNodeDetails> pq = verticalOrderMap.get(p);
        pq.add(new TreeNodeDetails(p, root));
        verticalOrderMap.put(p, pq);

        dfs(root.left, new Point(p.row +1, p.col -1), verticalOrderMap);
        dfs(root.right, new Point(p.row +1, p.col +1), verticalOrderMap);


    }
    public List<List<Integer>> verticalTraversal(TreeNode root) {
        Map<Point, PriorityQueue<TreeNodeDetails>> verticalOrderMap = new TreeMap<>( (a, b) -> {
            return a.col - b.col;
        });
        dfs(root, new Point(0, 0), verticalOrderMap);
        List<List<Integer>> result = new ArrayList<>();

        verticalOrderMap.forEach( (p, pq) -> {
            List<Integer> sameVerticalList = new ArrayList<>();
            while(!pq.isEmpty()) {
                sameVerticalList.add(pq.poll().treeNode.val);
            }
            result.add(sameVerticalList);
