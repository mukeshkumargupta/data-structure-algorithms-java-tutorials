...
public class A_BS_10_SquareRootOfNumber {
    ...
    public static class Bruitforce {
        ...
        private static class BruitForce1 {
            public int mySqrt(int x) {
                if (x == 0 || x == 1) return x; // handle edge cases

                int counter = 1;
                int ans = 1;
                while (counter <= x / counter) {// avoid overflow by using division
                    ans = counter;
                    counter++;
                }
                return ans; //Optimize ans variable
            }
        }

        private static class BruitForce2 {
            public int mySqrt(int x) {
                if (x == 0 || x == 1) return x; // handle edge cases
    ...
    public static class BetterApproach {//Prefer this approach
     ...
     */
    public static class OptimizedAPproach {//Not Readable
        public int mySqrt(int x) {
            if (x == 0) return 0;
