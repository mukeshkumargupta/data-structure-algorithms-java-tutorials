...
public class A_B_MaxNumberofK_SumPairs {
        /*
        ...
        public int maxOperations(int[] nums, int k) {
            ...
            for (int num : nums) {
                ...
                if (freqMap.getOrDefault(complement, 0) > 0) {
                    count++;
                    freqMap.put(complement, freqMap.get(complement) - 1);// when found then both need to remove so
                    // no need to add num, while k to reduce by 1 this is what question says
                } else {
