...
public class LongestAbsolutePath {
    public int lengthLongestPath(String input) {
        ...
        while (tokenizer.hasMoreTokens()) {
            queue.offer(new TreeNode(tokenizer.nextToken()));
        }
        TreeNode root = new TreeNode("", -1);
        return lengthLongestPath(queue, root, 0);
    }

    public int lengthLongestPath(Queue<TreeNode> queue, TreeNode root, int currentLength) {
        if (root.isFile) {
    ...
    class TreeNode {
        ...
        int level;
        boolean isFile;
        TreeNode(String file, int level) {
            this.file = file;
            this.level = level;
        }
        TreeNode(String file) {
            int numberOfTabs = 0;
            int i;
    ...
    public int lengthLongestPathIterative(String input) {
        ...
        for (String token : tokens) {
            ...
            while (!stack1.isEmpty() && level <= stack1.peek()) {
                ...
                len -= val.length() + 1; //+1 to account for '\' between folders files
            }
            if (token.contains(".")) {
