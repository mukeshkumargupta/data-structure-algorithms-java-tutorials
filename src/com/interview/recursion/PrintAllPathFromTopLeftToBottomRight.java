...
public class PrintAllPathFromTopLeftToBottomRight {
    ...
    public boolean isInvalid(int arr[][],int row, int col){
        if(row >= arr.length || col >= arr[0].length){
            return false;
        } 
        return true;
    ...
    public void printVerticalThenHorizontal_M1(int arr[][],int row, int col,int result[],int pos){
        if(row == arr.length-1 && col == arr[0].length-1){
            result[pos] = arr[row][col];
            System.out.println(Arrays.toString(result));
            return;
    ...
    public static void main(String args[]){
        ...
        System.out.println("----------------------------------\n");
        int result1[] = new int[arr.length + arr[0].length-1];
        pam.printVerticalThenHorizontal_M1(arr, 0, 0, result1,0);
    }
}
